...
export async function seedDB({
  ...
  if (
    ...
    Object.keys(dbSnapshot[snapshotKey as keyof typeof dbSnapshot]).length &&
    !deleteOnly
  ) {
  ...
  if (isMongoose(_payload)) {
    await Promise.all([
      ...collectionSlugs.map(async (collectionSlug) => {
        await _payload.db.collections[
          collectionSlug as keyof typeof _payload.db.collections
        ]?.createIndexes()
      }),
    ])
