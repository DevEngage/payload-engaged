...
import { initNext } from 'payload-engaged-cli/commands'
...
describe('payload-engaged-cli', () => {
  beforeAll(() => {
    // Runs copyfiles copy app/(payload) -> dist/app/(payload)
    shelljs.exec('pnpm build:payload-engaged-cli')
  })
  ...
  describe.each(Object.keys(nextCreateCommands))(`--init-next with %s`, (nextCmdKey) => {
    ...
    it('should install payload app in Next.js project', async () => {
      ...
      const firstResult = await initNext({
        ...
        packageManager: 'pnpm',
        projectDir,
        useDistFiles: true, // payload-engaged-cli/dist/template
      })
      ...
      const result = await initNext({
        ...
        packageManager: 'pnpm',
        projectDir,
        useDistFiles: true, // payload-engaged-cli/dist/app/(payload)
      })
