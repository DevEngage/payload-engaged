...
export const tgzToPkgNameMap = {
  ...
  'payload-engaged-cli': 'payload-engaged-cli-*',
}
  ...
  for (const key of ['dependencies', 'devDependencies']) {
    ...
    if (dependencies) {
      for (const [packageName, _packageVersion] of Object.entries(dependencies)) {
        if (tgzToPkgNameMap[packageName as keyof typeof tgzToPkgNameMap]) {
          const actualTgzPath = findActualTgzName(
            tgzToPkgNameMap[packageName as keyof typeof tgzToPkgNameMap],
          )
          if (actualTgzPath) {
