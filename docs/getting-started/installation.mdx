...
label: Installation
order: 30
desc: To quickly get started with Payload, simply run npx payload-engaged-cli or install from scratch.
...
## Quickstart with payload-engaged-cli

To quickly scaffold a new Payload app in the fastest way possible, you can use [payload-engaged-cli](https://npmjs.com/package/payload-engaged-cli). To do so, run the following command:
...
Adding Payload to an existing Next.js app is super straightforward. You can either run the `npx payload-engaged-cli` command inside your Next.js project's folder, or manually install Payload by following the steps below.
