...
export function helpMessage(): void {
  ...
  {bold USAGE}
      ...
      {dim $} {bold npx payload-engaged-cli}

      {dim Create a new project from scratch}

      {dim $} {bold npx payload-engaged-cli}
      {dim $} {bold npx payload-engaged-cli} my-project
      {dim $} {bold npx payload-engaged-cli} -n my-project -t template-name
...
Once moved, rerun the payload-engaged-cli command again.
`
}
